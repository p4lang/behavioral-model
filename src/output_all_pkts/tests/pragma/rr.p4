...
struct headers {
    ...
    ipv4_t        ipv4;
    tcp_t         tcp;
    IPv4_up_to_ihl_only_h ipv4_up_to_ihl_only;
}
                 ...
                 inout standard_metadata_t stdmeta)
{
    ...
    action foo2(IPv4Address srcAddr, IPv4Address dstAddr) {
    ...
    table t2 {
        ...
        size = 16;
        implementation = action_selector(HashAlgorithm.path_permutation, 16, 4);
    }
    apply {
        meta.hash1 = 456;
        t2.apply();
                  ...
                  in headers hdr)
{
    apply {
        ...
        packet.emit(hdr.tcp);
        packet.emit(hdr.ipv4_up_to_ihl_only);
    }
}
