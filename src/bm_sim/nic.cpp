...
packet_handler(int port_num, const char *buffer, int len, void *cookie) {
  // static_cast<NicWContexts *> if okay here because cookie was obtained by
  // casting a NicWContexts * to void *
  std::cout << "packet_handler begin" << "\n\n\n\n";
  static_cast<NicWContexts *>(cookie)->receive(port_num, buffer, len);
  std::cout << "packet_handler end" << "\n\n\n\n";
}

// TODO(antonin): maybe a factory method would be more appropriate for Nic
NicWContexts::NicWContexts(size_t nb_cxts, bool enable_swap)
  : DevMgr(),
    nb_cxts(nb_cxts), contexts(nb_cxts), enable_swap(enable_swap),
    phv_source(PHVSourceIface::make_phv_source(nb_cxts)) {
  std::cout << "nb_cxts : " << nb_cxts << "\n\n";
  for (size_t i = 0; i < nb_cxts; i++) {
    std::cout << "i : " << i << "\n\n";
    contexts.at(i).set_cxt_id(i);
  }
  std::cout << "nb_cxts complete" << "\n\n";
}

LookupStructureFactory NicWContexts::default_lookup_factory {};

int
NicWContexts::receive(port_t port_num, const char *buffer, int len) {
...
void
NicWContexts::start_and_return() {
  {
    std::unique_lock<std::mutex> config_lock(config_mutex);
    if (!config_loaded && !enable_swap) {
      Logger::get()->error(
          "The Nic was started with no P4 and config swap is disabled");
    }
...
void
NicWContexts::reset_target_state() {
  reset_target_state_();
}

void
NicWContexts::swap_notify() {
  swap_notify_();
}

std::string
NicWContexts::get_debugger_addr() const {
#ifdef BM_DEBUG_ON
...
std::string
NicWContexts::get_event_logger_addr() const {
  return event_logger_addr;
}

void
NicWContexts::enable_config_swap() {
  enable_swap = true;
}

void
NicWContexts::disable_config_swap() {
  enable_swap = false;
}

void
NicWContexts::add_required_field(const std::string &header_name,
                                  const std::string &field_name) {
  required_fields.insert(std::make_pair(header_name, field_name));
}

void
NicWContexts::force_arith_field(const std::string &header_name,
                                   const std::string &field_name) {
  arith_objects.add_field(header_name, field_name);
}

void
NicWContexts::force_arith_header(const std::string &header_name) {
  arith_objects.add_header(header_name);
}

bool
NicWContexts::set_group_selector(
    cxt_id_t cxt_id,
...
int
NicWContexts::init_objects(std::istream *is, device_id_t dev_id,
...
int
NicWContexts::init_objects(const std::string &json_path, device_id_t dev_id,
                              std::shared_ptr<TransportIface> transport) {
...
int
NicWContexts::init_objects_empty(device_id_t dev_id,
                                    std::shared_ptr<TransportIface> transport) {
  return init_objects(nullptr, dev_id, transport);
}

int
NicWContexts::init_from_command_line_options(
...
int
NicWContexts::init_from_options_parser(
  ...
  if (parser.file_logger != "") {
    std::cout << "logger file: " << parser.file_logger << "\n\n\n\n";
  ...
  std::cout << "before set_packet_handler" << "\n\n\n\n";
  set_packet_handler(packet_handler, static_cast<void *>(this));
  std::cout << "after set_packet_handler" << "\n\n\n\n";
...
NicWContexts::load_new_config(const std::string &new_config) {
...
RuntimeInterface::ErrorCode
NicWContexts::serialize(std::ostream *out) {
  std::unique_lock<std::mutex> config_lock(config_mutex);
...
int
NicWContexts::deserialize(std::istream *in) {
...
int
NicWContexts::deserialize_from_file(const std::string &state_dump_path) {
...
void
NicWContexts::block_until_no_more_packets() {
...
std::string
NicWContexts::get_config() const {
  std::unique_lock<std::mutex> config_lock(config_mutex);
  return current_config;
}

std::string
NicWContexts::get_config_md5_() const {
  MD5_CTX cxt;
  MD5_Init(&cxt);
...
std::string
NicWContexts::get_config_md5() const {
  std::unique_lock<std::mutex> config_lock(config_mutex);
  return get_config_md5_();
}

P4Objects::IdLookupErrorCode
NicWContexts::p4objects_id_from_name(
...
CustomCrcErrorCode
NicWContexts::set_crc16_custom_parameters(
...
CustomCrcErrorCode
NicWContexts::set_crc32_custom_parameters(
    cxt_id_t cxt_id, const std::string &calc_name,
...
NicWContexts::new_packet_ptr(cxt_id_t cxt_id, port_t ingress_port,
...
Packet
NicWContexts::new_packet(cxt_id_t cxt_id, port_t ingress_port,
                            packet_id_t id, int ingress_length,
                            // NOLINTNEXTLINE(whitespace/operators)
                            PacketBuffer &&buffer) {
  boost::shared_lock<boost::shared_mutex> lock(process_packet_mutex);
...
int
NicWContexts::transport_send_probe(uint64_t x) const {
  struct msg_t {
    char sub_topic[4];
    s_device_id_t nic_id;
    uint64_t x;
...
Nic::Nic(bool enable_swap)
    : NicWContexts(1u, enable_swap) { }

std::unique_ptr<Packet>
Nic::new_packet_ptr(port_t ingress_port,
                       packet_id_t id, int ingress_length,
                       // NOLINTNEXTLINE(whitespace/operators)
                       PacketBuffer &&buffer) {
...
Packet
Nic::new_packet(port_t ingress_port, packet_id_t id, int ingress_length,
                   // NOLINTNEXTLINE(whitespace/operators)
                   PacketBuffer &&buffer) {
