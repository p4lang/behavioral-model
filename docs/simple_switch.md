...
is written to this field. Otherwise, this field is set to 0. If priority queueing is enabled, the qid also describe the priority level of each queue. Starting with 0 that has the highest priority until number_of_priority_queues - 1 that has the lowest priority. The number of priority queues for each port can be defined by adding --priority-queues when running ```simple_switch```.
