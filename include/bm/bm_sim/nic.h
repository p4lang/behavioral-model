...
//! @file nic.h
//! This file contains 2 classes: bm::NicWContexts and bm::Nic. When
//! implementing your target, you need to subclass one of them. By subclassing
//! bm::NicWContexts, you will be able to write a target containing an
...
//! provide the bm::Nic class. The bm::Nic class inherits from
//! bm::NicWContexts. Because it assumes that your switch will only use a
//! single bm::Context, the very notion of context can be removed from the
//! bm::Nic class and its dataplane APIs. However, because we offer unified
//! runtime APIs, you will have to use a context id of `0` when programming the
//! tables, even when your nic class inherits from bm::Nic and not
//! bm::NicWContexts.
...
//! bm::NicWContexts::receive_(int port_num, const char *buffer, int len) and
//! bm::NicWContexts::start_and_return_(). Your receive() implementation will
...
//!
//! Both nic classes support live swapping of P4-JSON configurations. To
//! enable it you need to provide the correct flag to the constructor (see
//! bm::NicWContexts::NicWContexts()). Swaps are ordered through the
//! runtime interfaces. We ensure that during the actual swap operation
//! (bm::NicWContexts::do_swap() method), there is no Packet instance
//! inflight, which we achieve using the process_packet_mutex mutex). The final
//! step of the swap is to call bm::NicWContexts::swap_notify_(), which
...
#ifndef BM_BM_SIM_NIC_H_
#define BM_BM_SIM_NIC_H_
...
//! required.
class NicWContexts : public DevMgr, public RuntimeInterface {
  friend class Nic;

 public:
  //! To enable live swapping of P4-JSON configurations, enable_swap needs to be
  //! set to `true`. See nic.h documentation for more information on
  //! configuration swap.
  explicit NicWContexts(size_t nb_cxts = 1u, bool enable_swap = false);
  ...
  Context *get_context(cxt_id_t cxt_id = 0u) {
    ...
    Context* context = &contexts.at(cxt_id);
    // std::cout << "The context associated with id " << cxt_id << " is " << *context << std::endl;
  ...
  //! Initialize the nic using command line options. This function is meant
  //! to be called right after your nic instance has been constructed. For
  ...
  //! plane. See nic.h documentation for more information.
  int swap_requested();
  ...
  //! be using. See nic.h documentation for more information.
  int do_swap();
 ...
 private:
  ...
  //! instantiate these objects (bm::NicWContexts::new_packet_ptr() and
  //! bm::NicWContexts::new_packet()).
...
//! Convenience subclass of NicWContexts for targets with a single
//! Context. This is the base class for the standard simple switch target
//! implementation.
class Nic : public NicWContexts {
 public:
  //! See NicWContexts::NicWContexts()
  explicit Nic(bool enable_swap = false);

  // to avoid C++ name hiding
  using NicWContexts::field_exists;
  ...
  // to avoid C++ name hiding
  using NicWContexts::new_packet_ptr;
  //! Convenience wrapper around NicWContexts::new_packet_ptr() for a single
  //! context switch.
  ...
  // to avoid C++ name hiding
  using NicWContexts::new_packet;
  //! Convenience wrapper around NicWContexts::new_packet() for a single
  //! context switch.
  ...
  //! performed by the target. See nic.h documentation for details.
  FieldList *get_field_list(const p4object_id_t field_list_id) {
    std::cout << "get field list" << "\n\n";
