...
class PsaSwitchGrpcTest_gNMI : public PsaSwitchGrpcBaseTest {
 protected:
  using StreamType =
      grpc::ClientReaderWriter<gnmi::SubscribeRequest, gnmi::SubscribeResponse>;

  PsaSwitchGrpcTest_gNMI()
...
TEST_F(PsaSwitchGrpcTest_gNMI, PortOperStatusUpdates) {
  const int port = 1;
...
TEST_F(PsaSwitchGrpcTest_gNMI, PortCounters) {
  const int port = 1;
  const std::string iface_name = make_iface_name(port, "eth1");
  EXPECT_TRUE(create_iface(iface_name, true).ok());
  EXPECT_TRUE(set_port_oper_status(port, p4::bm::OPER_STATUS_UP).ok());
