...
class PsaSwitchGrpcTest_IdleTimeout : public PsaSwitchGrpcBaseTest {
 protected:
  PsaSwitchGrpcTest_IdleTimeout()
      : PsaSwitchGrpcBaseTest(digest_proto),
...
PsaSwitchGrpcTest_IdleTimeout::defaultTimeout;

TEST_F(PsaSwitchGrpcTest_IdleTimeout, EntryExpire) {
...
TEST_F(PsaSwitchGrpcTest_IdleTimeout, NotifyAgain) {
  const std::string smac("\x11\x22\x33\x44\x55\x66");
  const std::chrono::milliseconds idle_timeout{2000};
  auto entry = make_entry(smac, idle_timeout);
  EXPECT_TRUE(insert(entry).ok());
...
TEST_F(PsaSwitchGrpcTest_IdleTimeout, ReadEntry) {
  const std::string smac("\x11\x22\x33\x44\x55\x66");
  const std::chrono::milliseconds idle_timeout{2000};
  auto entry = make_entry(smac, idle_timeout);
  EXPECT_TRUE(insert(entry).ok());
...
TEST_F(PsaSwitchGrpcTest_IdleTimeout, ModifyTTL) {
  const std::string smac("\x11\x22\x33\x44\x55\x66");
