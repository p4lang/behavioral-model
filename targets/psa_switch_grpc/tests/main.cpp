...
class PsaSwitchGrpcEnv : public ::testing::Environment {
 public:
  // We make the switch a shared resource for all tests. This is mainly because
  // psa_switch detaches threads.
  ...
  void SetUp() override {
    auto &runner = PsaSwitchGrpcRunner::get_instance(
        true, PsaSwitchGrpcBaseTest::grpc_server_addr,
        PsaSwitchGrpcBaseTest::cpu_port,
        PsaSwitchGrpcBaseTest::dp_grpc_server_addr);
  ...
  void TearDown() override {
    PsaSwitchGrpcRunner::get_instance().shutdown();
  }
};
...
int main(int argc, char *argv[]) {
  ...
  ::testing::AddGlobalTestEnvironment(
       new pswitch_grpc::testing::PsaSwitchGrpcEnv);
