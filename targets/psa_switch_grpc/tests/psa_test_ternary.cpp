...
class PsaSwitchGrpcTest_Ternary : public PsaSwitchGrpcBaseTest {
 protected:
  PsaSwitchGrpcTest_Ternary()
      : PsaSwitchGrpcBaseTest(ternary_proto),
...
TEST_F(PsaSwitchGrpcTest_Ternary, ReadEntry) {
  int priority = 128;
...
TEST_F(PsaSwitchGrpcTest_Ternary, PriorityOrder) {
...
TEST_F(PsaSwitchGrpcTest_Ternary, PriorityZero) {
  int priority = 0;
  std::string pkt("\xaa\xbb");
  p4::bm::PacketStreamResponse rcv_pkt;
  auto entity = make_entry("\xaa\xbb", "\xff\xff", priority, a_s1_id);
