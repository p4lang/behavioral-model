    ...
    while (this->stream->Read(&request)) {
      ...
      if (!pkt_handler) continue;
      {
        ...
        if (request.id() != 0) {
          ...
          packet_id_translation[bm::psa::PsaSwitch::get_packet_id()] = request.id();
        }
...
// notifications & learning notifications. psa_switch_grpc "captures" these
...
// psa_switch_grpc usually disable nanomsg anyway.
...
PsaSwitchGrpcRunner::PsaSwitchGrpcRunner(
    bool enable_swap,
    ...
    int l1_max_entries,
    int l2_max_entries)
    : psa_switch(new bm::psa::PsaSwitch(enable_swap)),
...
int
PsaSwitchGrpcRunner::init_and_start(const bm::OptionsParser &parser) {
  ...
  if (!dp_grpc_server_addr.empty()) {
    ...
    my_dev_mgr.reset(dp_service);
      ...
      sysrepo_driver = std::unique_ptr<SysrepoDriver>(new SysrepoDriver(
          parser.device_id, psa_switch.get()));
#endif  // WITH_SYSREPO
  ...
  auto my_transport = std::make_shared<NotificationsCapture>(
      psa_switch.get());
  int status = psa_switch->init_from_options_parser(
  ...
  // using PortStatus = bm::DevMgrIface::PortStatus;
  // auto port_cb = std::bind(&PsaSwitchGrpcRunner::port_status_cb, this,
  //                          std::placeholders::_1, std::placeholders::_2);
  // psa_switch->register_status_cb(PortStatus::PORT_ADDED, port_cb);
  // psa_switch->register_status_cb(PortStatus::PORT_REMOVED, port_cb);
  ...
  auto transmit_fn = [this](bm::DevMgrIface::port_t port_num,
    ...
    if (cpu_port > 0 && port_num == cpu_port) {
      ...
      if (psa_switch->dump_packet_data > 0) {
        bm::Logger::get()->info(
            "Sending packet-in of length {} on port {}: {}",
            len, cpu_port, psa_switch->sample_packet_data(buf, len));
      }
      auto status = pi_packetin_receive(psa_switch->get_device_id(),
      ...
      psa_switch->transmit_fn(port_num, buf, len);
    }
  };
  psa_switch->set_transmit_fn(transmit_fn);

  bm::pi::register_switch(psa_switch.get(), cpu_port);
  ...
  bm_runtime::start_server(psa_switch.get(), thrift_port);
...
void
PsaSwitchGrpcRunner::wait() {
  PIGrpcServerWait();
}

void
PsaSwitchGrpcRunner::shutdown() {
  if (!dp_grpc_server_addr.empty()) dp_grpc_server->Shutdown();
  PIGrpcServerShutdown();
}

void
PsaSwitchGrpcRunner::block_until_all_packets_processed() {
  psa_switch->block_until_no_more_packets();
}

bool
PsaSwitchGrpcRunner::is_dp_service_active() {
...
void
PsaSwitchGrpcRunner::port_status_cb(bm::DevMgrIface::port_t port,
