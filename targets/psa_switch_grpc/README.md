...
# PsaSwitchGrpc - a version of PsaSwitch with P4Runtime support

This is an alternative version of the psa_switch target, which does not use
...
## Running psa_switch_grpc

For information on how to run psa_switch_grpc, use `psa_switch_grpc
...
psa_switch_grpc (to be precise, some of them are shared with the
psa_switch binary). When invoking psa_switch_grpc, these options will need
to be separated from standard bmv2 options using `--`.

Most users will invoke psa_switch_grpc as follows:
```
psa_switch_grpc --no-p4 \
...
* `--no-p4` is a standard bmv2 command-line option. It means that no JSON file
  is provided when starting psa_switch_grpc and bmv2 will not initially be
...
configure psa_switch_grpc with `--with-sysrepo` and build it again.
...
gRPC server running in the psa_switch_grpc process.
...
For psa_switch_grpc, we require that the interface name follow this pattern:
...
Enabling the Thrift runtime server (as in `psa_switch`) along the gRPC one,
...
#### I am using P4Runtime to program table entries, but I'd like to be able to use psa_switch_CLI to check the contents of my tables.

Refer to [Enabling the Thrift server](#enabling-the-thrift-server) above. Use
`--with-thrift` when configuring bmv2 and psa_switch_grpc.

#### What's the difference between the [PI folder in this repository](https://github.com/p4lang/behavioral-model/tree/main/PI) and the [PI repository](https://github.com/p4lang/PI)? How do they relate to psa_switch and psa_switch_grpc?
...
psa_switch processes - each with its own Thrift RPC server - using a single
...
convenient. For this reason, we have introduced psa_switch_grpc, a
slightly-modified version of psa_switch which includes a P4Runtime gRPC
...
(psa_switch_grpc), you can only have a single bmv2 device for a given
P4Runtime server. The PI C implementation used by psa_switch_grpc, which
...
We encourage newcomers to use psa_switch_grpc if possible.
