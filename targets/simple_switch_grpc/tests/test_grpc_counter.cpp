...
TEST_F(SimpleSwitchGrpcTest_Counter, CounterHit) {
  ...
  auto dc_id = get_dc_id(p4info, "cntr");
  ...
  exact->set_value(std::string("\x00\x00\x00\x0a", 4));
  ...
  *(update->mutable_entity()) = entity;
  ...
  // Add second table entry
  exact->set_value(std::string("\x00\x00\x00\x05", 4));
  *(update->mutable_entity()) = entity;
  {
    p4::WriteResponse write_response;
    ClientContext context;
  ...
  // Send a packet matching second entry
  packet_request.set_packet(std::string(5, '\xab'));
  {
    ClientContext context;
    auto stream = dataplane_stub->PacketStream(&context);
    stream->Write(packet_request);
  ...
  *(direct_counter_entry->mutable_table_entry()) = entity.table_entry();
  // Read counter of second entry
  {
    ClientContext context;
