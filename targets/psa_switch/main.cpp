...
main(int argc, char* argv[]) {
  ...
  std::string dp_grpc_server_addr;
  {
    ...
    if (rc != bm::TargetParserBasic::ReturnCode::OPTION_NOT_PROVIDED &&
        ...
        std::cerr << "Failed to parse --dp-grpc-server-addr\n";
        std::exit(1);
      }
  }
  ...
  std::string grpc_server_addr;
  {
    ...
    if (rc == bm::TargetParserBasic::ReturnCode::OPTION_NOT_PROVIDED) {
      ...
      std::cerr << "Failed to parse --grpc-server-addr\n";
      std::exit(1);
    }
  }
  ...
  bool grpc_server_ssl = false;
  {
    ...
    if (rc != bm::TargetParserBasic::ReturnCode::SUCCESS) {
      std::cerr << "Failed to parse --grpc-server-ssl\n";
      std::exit(1);
    }
  }
  ...
  std::string grpc_server_cacert;
  {
    ...
    if (rc == bm::TargetParserBasic::ReturnCode::OPTION_NOT_PROVIDED) {
      ...
      std::cerr << "Failed to parse --grpc-server-cacert\n";
      std::exit(1);
    }
  }
  ...
  std::string grpc_server_cert;
  {
    ...
    if (rc == bm::TargetParserBasic::ReturnCode::OPTION_NOT_PROVIDED) {
      ...
      std::cerr << "Failed to parse --grpc-server-cert\n";
      std::exit(1);
    }
  }
  ...
  std::string grpc_server_key;
  {
    ...
    if (rc == bm::TargetParserBasic::ReturnCode::OPTION_NOT_PROVIDED) {
      ...
      std::cerr << "Failed to parse --grpc-server-key\n";
      std::exit(1);
    }
  }
  ...
  bool grpc_server_with_client_auth = false;
  {
    ...
    if (rc != bm::TargetParserBasic::ReturnCode::SUCCESS) {
      std::cerr << "Failed to parse --grpc-server-with-client-auth\n";
      std::exit(1);
    }
  }
