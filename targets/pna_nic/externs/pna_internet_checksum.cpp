...
#include "pna_internet_checksum.h"
...
void
PNA_InternetChecksum::init() {
    clear();
}

void
PNA_InternetChecksum::get(Field &dst) const {
    dst.set(static_cast<uint16_t>(~sum));
}

void
PNA_InternetChecksum::get_state(Field &dst) const {
    dst.set(sum);
}

void
PNA_InternetChecksum::get_verify(Field &dst, Field &equOp) const {
    dst.set(equOp.get<uint16_t>() == static_cast<uint16_t>(~sum));
}

void
PNA_InternetChecksum::set_state(const Data &src) {
    sum = src.get<uint16_t>();
}

void
PNA_InternetChecksum::clear() {
    sum = 0;
}

void
PNA_InternetChecksum::add(const std::vector<Field> &fields) {
    Data input(0);
    Data chunk;
...
void
PNA_InternetChecksum::subtract(const std::vector<Field> &fields) {
    Data input(0);
    Data chunk;
    static const Data mask(0xffff);
...
BM_REGISTER_EXTERN_W_NAME_METHOD(InternetChecksum, PNA_InternetChecksum, get_state, Field &);
BM_REGISTER_EXTERN_W_NAME_METHOD(InternetChecksum, PNA_InternetChecksum, set_state,const Data &);
BM_REGISTER_EXTERN_W_NAME_METHOD(InternetChecksum, PNA_InternetChecksum, get, Field &);
BM_REGISTER_EXTERN_W_NAME_METHOD(InternetChecksum, PNA_InternetChecksum, get_verify, Field &, Field &);
BM_REGISTER_EXTERN_W_NAME_METHOD(InternetChecksum, PNA_InternetChecksum, clear);
