...
class PnaNic : public Nic {
 ...
 public:
  ...
  read_counters(cxt_id_t cxt_id,
    ...
    auto *counter = static_cast<PNA_Counter*>(ex);
  ...
  reset_counters(cxt_id_t cxt_id,
    ...
    if (!ex) return Counter::CounterErrorCode::INVALID_COUNTER_NAME;
    PNA_Counter *counter = static_cast<PNA_Counter*>(ex);
  ...
  meter_array_set_rates(
    ...
    auto *meter = static_cast<PNA_Meter*>(ex);
 ...
 private:
  // static constexpr size_t nb_main_threads = 4u;
  // static constexpr port_t PSA_PORT_LOOPBACK = 0xfffffffa;
  ...
  // struct MainThreadMapper {
  //   explicit MainThreadMapper(size_t nb_threads)
  //       : nb_threads(nb_threads) { }

  //   size_t operator()(size_t egress_port) const {
  //     return input_port % nb_threads;
  //   }
