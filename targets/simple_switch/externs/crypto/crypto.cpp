...
class ExternCrypt : public ExternType {
 public:
  ...
  void protect( const Data &in_sak,
    ...
    std::vector<unsigned char> secure_association_key = get_char_vector(in_sak.get_string(), SAK_SIZE);
    //cout << "[p4sec] secure_association_key" << std::endl;
    //hexdump((char*)&secure_association_key[0], secure_association_key.size());

    std::vector<unsigned char> secure_channel_identifier = get_char_vector(in_sci.get_string(), SCI_SIZE);
    //cout << "[p4sec] secure_channel_identifier" << std::endl;
    //hexdump((char*)&secure_channel_identifier[0], secure_channel_identifier.size());

    std::vector<unsigned char> packet_number = get_char_vector(in_pn.get_string(), PN_SIZE);
    //cout << "[p4sec] packet_number" << std::endl;
    //hexdump((char*)&packet_number[0], packet_number.size());

    std::vector<unsigned char> source_mac_address = get_char_vector(in_src_addr.get_string(), ADDR_SIZE);
    //cout << "[p4sec] source_mac_address" << std::endl;
    //hexdump((char*)&source_mac_address[0], source_mac_address.size());

    std::vector<unsigned char> destionation_mac_address = get_char_vector(in_dst_addr.get_string(), ADDR_SIZE);
    //cout << "[p4sec] destionation_mac_address" << std::endl;
    //hexdump((char*)&destionation_mac_address[0], destionation_mac_address.size());

    std::vector<unsigned char> security_tag = get_char_vector(in_sectag.get_string(), SECTAG_SIZE);
    //cout << "[p4sec] security_tag" << std::endl;
    //hexdump((char*)&security_tag[0], security_tag.size());

    std::vector<unsigned char> ethertype = get_char_vector(in_ethertype.get_string(), ETHERTYPE_SIZE);
    ...
    protection_function(secure_association_key, secure_channel_identifier, packet_number, destionation_mac_address, source_mac_address,
  ...
  void validate(const Data &in_sak,
    ...
    int valid = validation_function(secure_association_key, secure_channel_identifier, packet_number, destionation_mac_address, source_mac_address,
    ...
    get_packet().remove(get_packet().get_data_size());
    //make room for the ciphertext and write the ciphertext in it
    char *payload_start = get_packet().prepend((unsigned long int) user_data.size());
  ...
  void protection_function(std::vector<unsigned char> secure_association_key, //
    ...
    EVP_EncryptUpdate(e_ctx, &out_secure_data[0], &actual_size, &P[0], P.size() );
    EVP_EncryptFinal(e_ctx, &out_secure_data[actual_size], &final_size);
    EVP_CIPHER_CTX_ctrl(e_ctx, EVP_CTRL_GCM_GET_TAG, 16, &out_integrity_check_value[0]);
  ...
  int validation_function(std::vector<unsigned char> secure_association_key, //
      ...
      EVP_DecryptUpdate(d_ctx, &out_user_data[0], &actual_size, &P[0], P.size() );
      EVP_CIPHER_CTX_ctrl(d_ctx, EVP_CTRL_GCM_SET_TAG, 16, &integrity_check_value[0]);
